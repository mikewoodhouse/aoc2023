...
class Argh:
    value: str
    damaged: list[int]
    to_hash: Argh | None = None
    to_dot: Argh | None = None

    def __post_init__(self) -> None:
        if "?" in self.value:
            self.to_hash = Argh(value=self.value.replace("?", "#", 1), damaged=self.damaged)
            self.to_dot = Argh(value=self.value.replace("?", ".", 1), damaged=self.damaged)
    ...
    @staticmethod
    def from_line(s: str) -> Argh:
        l, r = s.split(" ")[:2]
        damaged = [int(s) for s in r.split(",")] * 5
        base = "?".join([l] * 5)
        return Argh(value=base, damaged=damaged)
...
def part2(lines: list[str]) -> int:
    ...
    for line in lines:
        print(line)
        tot += Argh.from_line(line).arrangements
    return tot
